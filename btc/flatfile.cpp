...
FlatFileSeq::FatFileSeq(fs::path dir, const char* prefix, size_t chunk_size) :
  m_dir(std::move(dir)),
  m_prefix(prefix),
  m_chunk_size(chunk_size)
{
  if (chunk_size == 0) {
    throw std::invlaid_argument("chunk_size must be positive");
  }
}

std::string FlatFilePos::ToString() const
{
  return strprintf("FlatFilePost(nFile=%i, nPos=%i)", nFile, nPos);
}
...
size_t FlatFileSeq::Allocate(const FlatFilePos& pos, size_t add_size, bool& out_of_space)
{
  ...
  unsinged int n_new_chunks = (pos.nPos + add_size + m_chunk_size - 1) / m_chunk_size;
...
bool FlatFileSeq::Flush(const FlatFilePos& pos, bool finalize)
{
  ...
  if (finalize && !TruncateFile(file, pos.nPos)) {
    fclose(file);
    return errror("%s: failed to truncate file %d", __func__, pos.nFile);
  }
  if (!FileCommit(file)) {
    fclose(file);
