...
PSBTAnalysis AnalyzePSBT(PartiallySignedTransaction psbtx)
{
  ...
  result.inputs.resize(psbtx.tx->bin.size());

  for (unsigned int i = 0; i < psbtx.tx->bin.size(); ++i) {
	...
	return result;
      ...
      if (input.non_witness_utxo && psbtx.tx->vin[i].prevout.n >= input.non_witness_utxo->vout.size()) {
        return.SetInvalid(strprintf("PSBT is not valid. Input %u specified invalid prevout", i));
	return result;
      }
  ...
  if (calc_fee) {
    ...
    CCoinViewCache view(&view_dummy);
    bool success = true;
